  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 w-full flex-1">
      <div className="flex flex-col lg:flex-row gap-12">
        ...
        {/* LEFT COLUMN: IMAGE & AI VISUALIZATION */}
        <div className="w-full lg:w-[55%]">
          <div className="sticky top-24">
            ...
            >
              ...
              {/* Overlay is always visible on detail page for clarity */}
              ...
              <motion.div 
                ...
                className="absolute bottom-6 right-6 bg-white/90 backdrop-blur-md px-5 py-3 rounded-2xl shadow-xl border border-gray-200 flex items-center space-x-4"
              >
                <div className="bg-accent-DEFAULT/10 p-2.5 rounded-xl">
                  <Ruler className="w-6 h-6 text-accent-DEFAULT" />
                </div>
                <div>
                  <p className="text-[10px] text-gray-500 font-bold uppercase tracking-wider">Est. Usable Area</p>
                  <p className="text-xl font-black text-primary">{listing.surface_area_cm2} cm²</p>
                </div>
              </motion.div>
            </motion.div>
          </div>
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4 }}>
            ...
            <div className="flex flex-wrap items-center gap-3 mb-6">
              ...
              <span className={`px-4 py-1.5 text-xs font-bold rounded-full uppercase tracking-widest ${
                listing.status === 'active' ? 'bg-emerald-100 text-emerald-700 border border-emerald-200' :
                listing.status === 'sold' ? 'bg-gray-100 text-gray-700 border border-gray-200' :
                'bg-amber-100 text-amber-700 border border-amber-200'
              }`}>
                {listing.status}
              </span>
              <span className="flex items-center text-sm text-gray-400 font-semibold ml-auto">
                <Calendar className="w-4 h-4 mr-1.5" />
                {new Date(listing.createdAt).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric'})}
              </span>
            </div>

            <h1 className="text-4xl lg:text-5xl font-black text-primary mb-6 tracking-tight leading-tight">
              {listing.title}
            </h1>
            
            <div className="flex items-baseline mb-10">
              <span className="text-6xl font-black text-accent-DEFAULT tracking-tighter">
              ...
              {[
                ...
                <div key={i} className="bg-gray-50 p-5 rounded-2xl border border-gray-100 hover:border-gray-200 transition-colors">
                  <div className="flex items-center space-x-3 mb-3">
                    ...
                    <p className="text-xs text-gray-500 font-bold uppercase tracking-wider">{metric.label}</p>
                  </div>
            ...
            <div className="mt-auto">
              {authLoading ? (
                ...
                <div className="p-6 bg-gray-50 border border-gray-200 rounded-2xl text-center">
                  <p className="text-gray-600 font-medium mb-4">You must be logged in to request this material.</p>
                  <Link href={`/auth?callbackUrl=/listing/${id}`} className="bg-primary text-white px-8 py-3 rounded-full font-bold shadow-md hover:bg-primary/90 transition-all inline-block">
                    Sign in to Purchase
                  </Link>
                </div>
              ) : user.uid === listing.sellerId ? (
                <div className="p-6 bg-blue-50 border border-blue-200 rounded-2xl text-center">
                  <p className="text-blue-800 font-bold text-lg mb-1">This is your listing.</p>
                  <p className="text-blue-600 font-medium text-sm">You can manage this item from your Seller Dashboard.</p>
                </div>
              ) : role === 'buyer' ? (
                <button 
                  onClick={() => setShowModal(true)} 
                  disabled={listing.status !== 'active'} 
                  className={`w-full py-5 rounded-2xl font-black text-xl flex items-center justify-center space-x-3 transition-all ${
                    listing.status === 'active' 
                      ? 'bg-primary text-white shadow-[0_8px_30px_rgba(26,26,46,0.2)] hover:bg-primary/90 hover:shadow-[0_12px_40px_rgba(26,26,46,0.3)] transform hover:-translate-y-1' 
                      : 'bg-gray-100 text-gray-400 cursor-not-allowed border border-gray-200'
                  }`}
                >
                  <span>{listing.status === 'active' ? 'Request to Purchase' : 'Listing Unavailable'}</span>
                  {listing.status === 'active' && <Send className="w-5 h-5" />}
                </button>
              ) : (
                <div className="p-5 bg-amber-50 border border-amber-200 rounded-2xl text-center">
                  <p className="text-amber-800 font-bold">Seller Accounts cannot purchase material.</p>
                </div>
              )}
            </div>
        ...
        {showModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
            ...
            <motion.div 
              ...
              className="bg-white w-full max-w-lg rounded-[2rem] p-8 relative z-10 shadow-2xl overflow-hidden"
            >
              <div className="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-accent-DEFAULT to-blue-400" />
              
              <button 
                onClick={() => !ordering && setShowModal(false)} 
                className="absolute top-6 right-6 p-2 bg-gray-50 text-gray-400 hover:text-gray-900 hover:bg-gray-100 rounded-full transition-colors"
                disabled={ordering}
              >
                <X className="w-5 h-5" />
              </button>
              ...
              <p className="text-gray-500 font-medium mb-6">
                Connect with the factory to arrange pickup and payment for <strong className="text-accent-DEFAULT">${listing.price}</strong>.
              </p>
              ...
              <div className="bg-gray-50 p-4 rounded-xl border border-gray-100 mb-6 flex items-center space-x-4">
                ...
                <div>
                  <p className="font-bold text-gray-900 text-sm truncate max-w-[250px]">{listing.title}</p>
              ...
              <form onSubmit={handleOrder}>
                ...
                <textarea 
                  ...
                  placeholder="Hi, I'm a local fabricator. I can pick this up tomorrow morning. Is the price flexible?"
                  className="w-full p-4 bg-white border border-gray-200 shadow-inner rounded-2xl focus:ring-2 focus:ring-accent-DEFAULT/20 focus:border-accent-DEFAULT outline-none font-medium mb-8 resize-none transition-all"
                  disabled={ordering}
                />
                ...
                <div className="flex space-x-3">
                  ...
                  <button 
                    type="submit" 
                    disabled={ordering || !message.trim()} 
                    className="flex-[2] bg-accent-DEFAULT text-white font-black py-4 rounded-xl hover:bg-accent-DEFAULT/90 disabled:opacity-50 transition-all shadow-lg shadow-accent-DEFAULT/20 flex items-center justify-center"
                  >
                    {ordering ? <LoadingSpinner size="sm" /> : (
                      <>
