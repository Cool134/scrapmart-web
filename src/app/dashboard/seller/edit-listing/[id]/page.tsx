...
import { getListing, updateListing, addListingImage, getListingImages } from '@/lib/firestore';
import { uploadScrapImage } from '@/lib/storage';
import { useAuthState } from '@/lib/auth';
import { Camera, Loader2, UploadCloud, ChevronLeft, Sparkles, CheckCircle2, TrendingUp, Image as ImageIcon, Trash } from 'lucide-react';
...
const EditListingPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchListingData = async () => {
      ...
      try {
        ...
        if (fetchedListing && fetchedListing.sellerId === user.uid) {
          setFormData({
          ...
          setNumericalData({
            weight: fetchedListing.weight.toString(),
            thickness: fetchedListing.thickness.toString(),
            pricePerKg: fetchedListing.pricePerKg.toString(),
          });
          setAiConfidence(fetchedListing.aiConfidence || null);
          setAiDetectedMaterial(fetchedListing.materialType);
          setAiDetectedDescription(fetchedListing.description);
          setAiDetectedThickness(fetchedListing.thickness || null);

          const images = await getListingImages(listingId);
          setExistingImages(images);
        } else {
          toast.error("Listing not found or you don't have permission to edit.");
  ...
  const handleUpdateListing = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!user || !listingId) {
      toast.error("Authentication error or invalid listing ID.");
      return;
    }

    setSubmitting(true);
    toast.loading("Updating listing...", { id: 'update-listing' });

    try {
      let imageUrlToSave: string | undefined;
      if (newSelectedFile) {
        imageUrlToSave = await uploadScrapImage(user.uid, newSelectedFile);
        // If there were existing images, we might want to delete the old primary or add this as a new one.
        // For now, simple replacement of primary image
        if (existingImages.length > 0) {
          // Optionally delete old image from storage here: deleteScrapImage(existingImages[0].imageUrl);
          // For MVP, just updating the reference or adding a new one.
        }
        await addListingImage({
          listingId: listingId,
          imageUrl: imageUrlToSave,
          order: existingImages.length + 1, // Add as next image
        });
      }

      const updatedListing: Partial<Omit<Listing, 'id' | 'createdAt'>> = {
      ...
      toast.success("Listing updated successfully!", { id: 'update-listing' });
      router.push('/dashboard/seller');
    } catch (error) {
      console.error("Error updating listing:", error);
      toast.error(`Failed to update listing: ${error instanceof Error ? error.message : 'Unknown error'}`, { id: 'update-listing' });
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleNumericalChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setNumericalData({ ...numericalData, [e.target.name]: e.target.value });
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="flex flex-col items-center">
          ...
          <p className="mt-3 text-lg text-gray-600">Loading listing data...</p>
        </div>
      </div>
    );
  }
    ...
    <AuthGuard role="seller">
      <div className="min-h-screen bg-[#F4F7FC] pb-24 text-gray-900 font-sans">
        ...
        <div className="sticky top-0 z-40 bg-white shadow-sm px-4 py-3 flex items-center gap-3">
          ...
          <h1 className="text-xl font-medium text-gray-800 tracking-tight">Edit Scrap Listing</h1>
        </div>
        ...
        <div className="max-w-xl mx-auto p-4 space-y-6 mt-2">
          ...
          {existingImages.length > 0 && (
            <div className="bg-white rounded-[24px] shadow-sm p-5 overflow-hidden transition-all duration-300">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="text-sm font-semibold text-gray-800">Existing Listing Images</h2>
              </div>
              <div className="grid grid-cols-2 gap-4">
                {existingImages.map((img, index) => (
                  <div key={img.id} className="relative w-full h-32 bg-gray-100 rounded-lg overflow-hidden group">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={img.imageUrl} alt={`Listing image ${index + 1}`} className="w-full h-full object-cover" />
          ...
          {/* AI Upload Section for new image */}
          <div className="bg-white rounded-[24px] shadow-sm p-5 overflow-hidden transition-all duration-300">
            <div className="flex items-center gap-2 mb-4">
              <Sparkles className="w-5 h-5 text-blue-600" />
              <h2 className="text-sm font-semibold text-gray-800">Add New Image & AI Analysis</h2>
            </div>
            
            <div 
              className="relative border-2 border-dashed border-gray-200 bg-[#F8FAFF] rounded-2xl flex flex-col items-center justify-center min-h-[150px] text-center cursor-pointer hover:bg-blue-50/50 transition-colors overflow-hidden"
            ...
            >
              ...
              {newPreviewImage ? (
                <div className="absolute inset-0 w-full h-full">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={newPreviewImage} alt="New scrap preview" className="w-full h-full object-cover" />
                ...
                <div className="p-8 flex flex-col items-center">
                  ...
                  <p className="text-sm font-medium text-gray-800 mb-1">Upload a new photo</p>
                  <p className="text-xs text-gray-500 max-w-xs">Our AI will analyze and update fields.</p>
                </div>
              )}
            </div>
          ...
          {/* Details Form */}
          <form onSubmit={handleUpdateListing} className="bg-white rounded-[24px] shadow-sm p-6 space-y-5">
